"""Appends fetched OHLC bars to a per-symbol/timeframe CSV file, skipping
bars already saved (by comparing against the last saved bar's time) so an
hourly cron job can safely re-fetch overlapping ranges without creating
duplicate rows.
"""
...
def csv_path(symbol: str, timeframe: str) -> Path:
    ...
    return HISTORY_DIR / f"{symbol}_{timeframe}.csv"
...
def append_bars(symbol: str, timeframe: str, bars: list[dict]) -> int:
