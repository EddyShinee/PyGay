...
//| MQL4 has no built-in socket API — same approach as the MT5 build.|
//| Windows only. Requires "Allow DLL imports" for the EA.           |
//+------------------------------------------------------------------+
#property strict
...
private:
   ...
   void PackSockAddr(const string host, const int port, uchar &addr[])
   {
      ...
      uchar host_bytes[];
      StringToCharArray(host, host_bytes);
...
public:
   ...
   bool     Send(const string text)
   {
      if(!m_connected)
         return false;
      uchar buf[];
      int total = StringToCharArray(text, buf, 0, WHOLE_ARRAY, CP_UTF8) - 1;
      int offset = 0;
      while(offset < total)
      {
         uchar chunk[];
